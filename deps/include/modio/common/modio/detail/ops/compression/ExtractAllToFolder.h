...
public:
	...
	void operator()(CoroType& Self, Modio::ErrorCode ec = {})
	{
		...
		{
			Modio::Detail::Logger().Log(Modio::LogLevel::Trace, Modio::LogCategory::Compression, "Extracting archive {}",
								Impl->ArchivePath.u8string());
			//Parsing archive contents is an async operation because it requires reading/seeking around in the archive file which is async
			...
			while (CurrentEntryIterator != Impl->ArchiveView.end())
			{
				...
				{
					//Otherwise, begin an async operation to extract that entry to a file with the path based on the root path
