...
public:
	StreamWrite(std::shared_ptr<Modio::Detail::FileObjectImplementation> IOObject, Modio::Detail::Buffer Buffer)
		: Buffer(std::move(Buffer)),
		  FileImpl(IOObject),
		  WriteOpParams {}
	{}

	StreamWrite(StreamWrite&& Other)
	...
	void operator()(CoroType& self, std::error_code ec = {})
	{
		...
		reenter(Coroutine)
		{
			...
			{
				Modio::FileOffset FileOffset = FileImpl->Tell();
