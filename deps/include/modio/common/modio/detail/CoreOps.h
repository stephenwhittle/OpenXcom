...
namespace Modio
{
	namespace Detail
	{
		namespace ComposedOps
		{
			...
			/// <param name="AllowCachedResponse">Ignored in the case of a request that's targetting a non-GET endpoint, and always disallow caching</param>
			...
			auto async_DownloadFile(Modio::Detail::HttpRequestParams DownloadParameters,
									...
									Modio::Optional < std::weak_ptr<Modio::ModProgressInfo>> ModProgress, CompletionTokenType &&
										Token)
			{
				return asio::async_compose<CompletionTokenType, void(Modio::ErrorCode)>(
					DownloadFile(DownloadParameters, DestinationPath,
								 Modio::Detail::Services::GetGlobalService<Modio::Detail::HttpService>().GetFileDownloadTicket(), ModProgress),
					Token, Modio::Detail::Services::GetGlobalContext().get_executor());
			}

			template<typename CompletionTokenType>
			auto async_ExtractAllFiles(Modio::filesystem::path ArchiveFile, Modio::filesystem::path DestinationPath, CompletionTokenType&& Token)
			{
