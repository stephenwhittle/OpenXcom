...
public:
	StreamRead(std::shared_ptr<Modio::Detail::FileObjectImplementation> IOObject, std::uintmax_t MaxBytesToRead,
			   Modio::Detail::DynamicBuffer Destination)
			...
			if (!ReadFile(FileImpl->GetFileHandle(), Buffer.Data(), Length, NULL, ReadOpParams.get()))
			{
				...
				if (Error != ERROR_IO_PENDING)
				{
					Modio::Detail::Logger().Log(Modio::LogLevel::Error, Modio::LogCategory::File,
										"Stream Read from {} failed, error = {}", FileImpl->GetPath().string(), Error);
