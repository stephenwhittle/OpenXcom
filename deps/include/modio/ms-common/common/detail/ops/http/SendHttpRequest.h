...
public:
	SendHttpRequest(std::shared_ptr<HttpRequestImplementation> Request, std::weak_ptr<HttpSharedStateBase> SharedState)
		: Request(Request),
		...
		reenter(CoroutineState)
		{
			...
			if (Request->Parameters.GetPayload())
			{
				...
				if (!WinHttpSendRequest(Request->RequestHandle, WINHTTP_NO_ADDITIONAL_HEADERS, 0,
										(void*) Payload.c_str(), Payload.length(), Payload.length(), (DWORD_PTR)ContextPtr))
				{
					auto err = GetLastError();
					Self.complete(Modio::make_error_code(Modio::HttpError::CannotOpenConnection));
					return;
