...
class InitializeHttp
{
public:
	InitializeHttp(std::wstring UserAgentString, std::shared_ptr<HttpSharedStateBase> SharedState)
	...
	void operator()(CoroType& Self, Modio::ErrorCode ec = {})
	{
		reenter(CoroutineState)
		{
			...
			*SharedState = HttpSharedStateBase(CurrentSession);
				WinHttpSetOption(CurrentSession, WINHTTP_OPTION_CONTEXT_VALUE, SharedState.get(),
								 sizeof(std::uintptr_t));
				WinHttpSetStatusCallback(
					CurrentSession, (WINHTTP_STATUS_CALLBACK)&ModioWinhttpStatusCallback,
