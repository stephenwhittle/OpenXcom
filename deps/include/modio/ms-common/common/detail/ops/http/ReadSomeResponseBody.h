...
public:
	ReadSomeResponseBody(std::shared_ptr<HttpRequestImplementation> Request, Modio::Detail::DynamicBuffer DataBuffer,
						 std::weak_ptr<HttpSharedStateBase> SharedState)
		: Request(Request),
...

			{
				...
				else if (Status == WinHTTPCallbackStatus::DataAvailable)
				{
					...
					if (ExtendedStatus.first != 0)
					{
						...
						if (WinHTTPCallbackStatus Status = PinnedState->FetchAndClearHandleStatus(Request->RequestHandle); 
							Status == WinHTTPCallbackStatus::RequestError)
						{
							Self.complete(Modio::make_error_code(Modio::HttpError::RequestError));
							return;
						}
