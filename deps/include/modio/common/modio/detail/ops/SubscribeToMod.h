...
namespace Modio
{
	namespace Detail
	{
		class SubscribeToMod
		{
		public:
			SubscribeToMod(Modio::GameID GameID, Modio::ApiKey ApiKey, Modio::ModID ModID)
				: GameID(GameID),
				  ApiKey(ApiKey),
				  ModId(ModID)
			{}
			...
			void operator()(CoroType& Self, Modio::ErrorCode ec = {})
			{
				reenter(CoroutineState)
				{
					...
					yield Modio::Detail::ComposedOps::async_PerformRequestAndGetResponse(
						ResponseBodyBuffer,
						Modio::Detail::SubscribeToModRequest.SetGameID(GameID).SetModID(ModId),
...

					{
						...
						Modio::Detail::SDKSessionData::GetSystemModCollection().AddOrUpdateMod(ProfileData);
						...
						else
						{
							...
							Self.complete(Modio::make_error_code(Modio::ModManagementError::AlreadySubscribed));
							return;
						}
					}
					...
					//No need to check return from SaveUserData, because the subscription is successful regardless of flushing the user storage state
					Self.complete({});
					return;
