...
class ReadSomeFromFileBuffered
{
...
public:
	ReadSomeFromFileBuffered(std::shared_ptr<Modio::Detail::FileObjectImplementation> IOObject, Modio::FileOffset Offset,
	...
	ReadSomeFromFileBuffered(ReadSomeFromFileBuffered&& Other)
	...
	void operator()(CoroType& self, std::error_code ec = {})
	{
		...
		reenter(Coroutine)
		{
			...
			Modio::Detail::Logger().Log(Modio::LogLevel::Trace, Modio::LogCategory::File, "Begin read of {} bytes from {}",
								Length, FileImpl->GetPath().string());
			...
			else
			{
				...
				*NumberOfBytesRead = ReadOpParams->InternalHigh;
				FileImpl->Seek(Modio::FileOffset(*NumberOfBytesRead),
							   Modio::Detail::SeekDirection::Forward);
