...
{
public:
	InitializeHttp(std::wstring UserAgentString, std::shared_ptr<HttpSharedState> SharedState)
	...
	void operator()(CoroType& Self, Modio::ErrorCode ec = {})
	{
		reenter(CoroutineState)
		{
			...
			auto SharedStatePtr = SharedState.get();
				auto set = WinHttpSetOption(CurrentSession, WINHTTP_OPTION_CONTEXT_VALUE, SharedStatePtr,
								 sizeof(std::uintptr_t));
			auto lasterror = GetLastError();
				WinHttpSetStatusCallback(
					CurrentSession, &ModioWinhttpStatusCallback,
